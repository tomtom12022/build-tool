...
if exist %backend_folder% (
    ...
    %git% clone https://esbscmg.testesunbank.com.tw/ts0135/abs_backend.git
    cd /d %backend_folder%
)
echo 更新完成！

:E_LOOP
set /p environment="請輸入目標環境? (DEV/SIT/UAT):"
if /i "%environment%" equ "DEV" echo. & goto CHECK_DEV_BUILD_DB
if /i "%environment%" equ "SIT" echo. & goto BUILD_SIT
if /i "%environment%" equ "UAT" echo. & goto BUILD_UAT
if /i "%environment%" equ "PROD" echo. & goto BUILD_PROD
goto :E_LOOP
...
call :GetDbScript "Create GLOBAL Table" ".\abs_backend\abs-db-script\global_edb\global_ddl\tables" "TB_.*.sql" 102_create_global_table.sql
call :GetDbScript "Create GLOBAL Index" ".\abs_backend\abs-db-script\global_edb\global_ddl\indexes" "TB_.*.sql" 103_create_global_index.sql
call :GetDbScript "Create GLOBAL Types" ".\abs_backend\abs-db-script\global_edb\global_ddl\types" "TT_.*.sql" 104_create_global_type.sql
call :GetDbScript "Create GLOBAL Package" ".\abs_backend\abs-db-script\global_edb\global_ddl\packages" "PKG_.*.PKG" 105_create_global_package.sql
call :GetDbScript "Create GLOBAL Package Body" ".\abs_backend\abs-db-script\global_edb\global_ddl\packages" "PKG_.*.PKB" 106_create_global_package_body.sql
call :GetDbScript "Create HK Table" ".\abs_backend\abs-db-script\hk\hk_ddl\tables" "TB_.*.sql" 107_create_table.sql
call :GetDbScript "Create HK Index" ".\abs_backend\abs-db-script\hk\hk_ddl\indexes" "TB_.*.sql" 108_create_index.sql
call :GetDbScript "Create HK Types" ".\abs_backend\abs-db-script\hk\hk_ddl\types" "TT_.*.sql" 109_create_type.sql
call :GetDbScript "Create HK Package" ".\abs_backend\abs-db-script\hk\hk_ddl\packages" "PKG_.*.PKG" 110_create_package.sql
call :GetDbScript "Create HK Package Body" ".\abs_backend\abs-db-script\hk\hk_ddl\packages" "PKG_.*.PKB" 111_create_package_body.sql
call :GetDbScript "Create HK dml" ".\abs_backend\abs-db-script\hk\hk_dml\initial-load" "TB_.*.sql" 112_create_hk_dml.sql
...
psql -c "\i start_build_db.sql" postgresql://enterprisedb:welcome1@DB的IP/abs_backup > result.txt 2>&1
...
call %lazytools_folder%\generate_spotbugs_report.bat
call %lazytools_folder%\collect_report_to_batfolder.bat
call %lazytools_folder%\summarize_report_info.bat
echo spotbugs執行完畢...
echo 刪除tmp後端...
...
if exist %frontend_folder% (
    ...
    %git% clone https://esbscmg.testesunbank.com.tw/ts0135/abs_frontend.git
    cd /d %frontend_folder%
)
%git% checkout remotes/origin/hk_dev_branch
echo 更新完成！
...
call :GetDbScript "Alter Hk Package" ".\abs_backend" "PKG_.*.PKG" 153_alter_hk_package.sql hk_ddl
call :GetDbScript "Alter Hk Package Body" ".\abs_backend" "PKG_.*.PKB" 154_alter_hk_package_body.sql hk_ddl
call :GetDbScript "Update Data" ".\abs_backend" "UPDATE_.*.sql" 155_update_table.sql db_change

call :createBuildScript "15.*.sql" start_build_db.sql
cd /d %bat_folder%
echo 請確認sql檔案是否有問題，沒有問題請繼續下一步
pause
if exist .\result.txt del /s /q .\result.txt && echo 刪除[result.txt]成功！ && echo.
...
psql -c "\i start_build_db.sql" postgresql://it20355:it20355esun13@DB的IP/abs > result.txt 2>&1
findstr /v /c:"NOTICE:" /c:"successfully completed" /c:"COMMENT" /c:"CREATE TABLE" /c:"CREATE INDEX" /c:"CREATE TYPE" /c:"DELETE 0" /c:"INSERT 0 1" /c:"CREATE PACKAGE" /c:"CREATE PACKAGE BODY" result.txt > error.txt
echo 執行完畢，請確認error.txt內容是否有問題，沒有問題請繼續下一步
pause

REM 上DMP SP
set environment=DMP
cd /d %bat_folder%
call :GetDbScript "Build Dmp FN" ".\abs_backend\abs-db-script\dmp\hk\dmp_ddl\functions" "FN_.*.sql" dmp_101_create_fn.sql
call :GetDbScript "Build Dmp SP" ".\abs_backend\abs-db-script\dmp\hk\dmp_ddl\stored-procedure" "SP_.*.sql" dmp_102_create_sp.sql
call :GetDbScript "Create start sql" "." "dmp_10.*.sql" start_build_dmp_db.sql
echo 請確認DMP sql檔案是否有問題，沒有問題請繼續下一步
pause
if exist .\result.txt del /s /q .\result.txt && echo 刪除[result.txt]成功！ && echo.
if exist .\error.txt del /s /q .\error.txt && echo 刪除[error.txt]成功！ && echo.
echo 執行過版DMP SIT DB...

psql -c "\i start_build_dmp_db.sql" postgresql://ts0135:ts0135@DB的IP/dmpt > result.txt 2>&1
findstr /v /c:"NOTICE:" /c:"successfully completed" /c:"COMMENT" /c:"CREATE TABLE" /c:"CREATE INDEX" /c:"CREATE TYPE" /c:"DELETE 0" /c:"INSERT 0 1" /c:"CREATE PACKAGE" /c:"CREATE PACKAGE BODY" /c:"GRANT" /c:"CREATE PROCEDURE" /c:"DROP PROCEDURE" result.txt > error.txt
echo 執行完畢，請確認error.txt內容是否有問題
...
psql -c "\i start_build_db.sql" postgresql://it20355:it20355esun13@DB的IP/absu > result.txt 2>&1
findstr /v /c:"NOTICE:" /c:"successfully completed" /c:"COMMENT" /c:"CREATE TABLE" /c:"CREATE INDEX" /c:"CREATE TYPE" /c:"DELETE 0" /c:"INSERT 0 1" /c:"CREATE PACKAGE" /c:"CREATE PACKAGE BODY" result.txt > error.txt

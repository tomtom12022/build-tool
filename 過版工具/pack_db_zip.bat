...
echo 打包checkmarx程式包 [%abs_db_build_zip%]...
@REM for /f %%i in ('dir %backend_folder%\abs-db-script /b ^| findstr /i "db_change | global_edb | hk"') do (
@REM    %_7z% a %abs_db_build_zip% %backend_folder%\abs-db-script\%%i -xr!target
@REM)

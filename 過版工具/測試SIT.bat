...
call :GetDbScript "Create HK Package Body" ".\abs_backend\abs-db-script\hk\hk_ddl\types" "TT_.*.sql" 104_create_type.sql
...
REM 上DMP SP
set environment=DMP
call :GetDbScript "Build Dmp" ".\abs_backend\abs-db-script\dmp\hk\dmp_ddl\functions" "FN_.*.sql" dmp_1_create_fn.sql
call :GetDbScript "Build Dmp" ".\abs_backend\abs-db-script\dmp\hk\dmp_ddl\stored-procedure" "SP_.*.sql" dmp_2_create_sp.sql
call :GetDbScript "Create start sql" "." "dmp_.*.sql" start_build_dmp_db.sql
cd /d %bat_folder%
echo 請確認sql檔案是否有問題，沒有問題請繼續下一步
pause
if exist .\result.txt del /s /q .\result.txt && echo 刪除[result.txt]成功！ && echo.
